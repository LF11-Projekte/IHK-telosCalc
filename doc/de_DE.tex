...
\begin{document}
	...
	Mit der Option Sprachausgabe kann eine Audioausgabe zur Navigationshilfe aktiviert werden. Dabei wird beim "Mit-der-Maus-Hovern" über Elementen in der Regel ein Text angesagt. Ebenfalls beim Wechseln der \textsc{GUI}-Elemente mittels der \texttt{Tabstopp}-Taste. Der Button \texttt{Noten Ansagen} wird zudem auch aktiviert. Bei dem Klicken auf dem Button werden alle Zwischennoten, der Gesamtnotendurchschnitt und das Prüfungsergebnis (also ob die Prüfung bestanden wurde) in der ausgewählten Zielsprache angesagt.
	
	Es kann durch alle Elemente, die zur Bedienung notwendig sind mittels der Tabulator-Taste "gegangen" werden. In das obere Menü gelangt man mittels der \texttt{[Alt]}-Taste -- in diesem kann man mit Pfeil-, Leer, und \texttt{Tabstopp}-Taste navigieren und Elemente mit \texttt{[ENTER]} bestätigen / auswählen. Mit der \texttt{[ESC]}-Taste kann man das obere Menü wieder verlassen.
	...
	\begin{description}
		...
		\item[TTS funktioniert nicht:] Starte aus dem Quellcode; Überprüfung der Konsolenmeldungen, ggf. Nachinstallation fehlender Pakete
	\end{description}
